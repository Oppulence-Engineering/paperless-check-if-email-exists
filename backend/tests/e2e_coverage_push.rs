...
mod v0_bulk_results_coverage {
	...
	async fn test_v0_bulk_results_csv_format() {
		...
		assert!(
			...
			"CSV should contain the email address"
		);
		assert!(
			body.contains("safe"),
			"CSV should contain is_reachable value"
		);
	}
	...
	async fn test_v0_bulk_results_json_with_pagination() {
		...
		let resp = request()
			.path(&format!("/v0/bulk/{}/results?format=json&limit=1", job_id))
			.method("GET")
			.reply(&routes)
			.await;
...
mod readiness_coverage {
	...
	async fn test_readyz_postgres_ok_no_worker() {
		...
		let resp = request().path("/readyz").method("GET").reply(&routes).await;
...
mod v1_bulk_results_csv_coverage {
	use crate::test_helpers::{
		insert_api_key, insert_job, insert_task, insert_tenant, safe_result, TestDb,
	};
	use reacher_backend::config::{BackendConfig, PostgresConfig, StorageConfig};
	use serial_test::serial;
	use std::sync::Arc;
