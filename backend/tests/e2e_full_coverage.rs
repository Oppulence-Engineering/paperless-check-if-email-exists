...
mod consume_coverage {
	...
	use reacher_backend::worker::consume::{setup_rabbit_mq, CHECK_EMAIL_QUEUE, MAX_QUEUE_PRIORITY};
	...
	async fn test_setup_connects_and_declares_queue() {
		let config = RabbitMQConfig { url: rmq_url(), concurrency: 4 };
	...
	async fn test_setup_sets_qos() {
		let config = RabbitMQConfig { url: rmq_url(), concurrency: 8 };
	...
	async fn test_setup_bad_url() {
		let config = RabbitMQConfig { url: "amqp://bad:9999".into(), concurrency: 1 };
...
mod do_work_coverage {
	...
	use reacher_backend::worker::do_work::{
		check_email_and_send_result, CheckEmailJobId, CheckEmailTask, TaskMetadata, RetryPolicy,
		TaskWebhook, Webhook,
	};
	...
	async fn test_check_email_returns_invalid_for_bad_syntax() {
		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "not-email".into(), ..Default::default()
			},
		...
		assert_eq!(result.is_reachable, check_if_email_exists::Reachable::Invalid);
	}
	...
	async fn test_check_email_with_metadata() {
		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "bad".into(), ..Default::default()
			},
	...
	async fn test_publish_consume_roundtrip() {
		...
		// Purge queue first
		let _ = channel.queue_purge(CHECK_EMAIL_QUEUE, QueuePurgeOptions::default()).await;

		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "roundtrip@test.com".into(), ..Default::default()
			},
			job_id: CheckEmailJobId::SingleShot,
			webhook: None,
			metadata: Some(TaskMetadata {
				tenant_id: None, request_id: None, correlation_id: None,
				created_by: None, retry_policy: None, dedupe_key: None, task_db_id: None,
			}),
		};
		let json = serde_json::to_vec(&task).unwrap();
		channel.basic_publish("", CHECK_EMAIL_QUEUE, BasicPublishOptions::default(), &json,
			BasicProperties::default().with_content_type("application/json".into()).with_priority(1),
		).await.unwrap().await.unwrap();

		let mut consumer = channel.basic_consume(CHECK_EMAIL_QUEUE, "dowork-con",
	...
	async fn test_task_with_webhook_no_url() {
		...
		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "noemail".into(), ..Default::default()
			},
			job_id: CheckEmailJobId::SingleShot,
			webhook: Some(TaskWebhook { on_each_email: None }),
			metadata: None,
		};
		let result = check_email_and_send_result(&task, None).await.unwrap();
		assert_eq!(result.is_reachable, check_if_email_exists::Reachable::Invalid);
...
mod shared_check_email_coverage {
	...
	async fn test_idempotency_key_reuses_cached_response() {
		...
		let req_body = serde_json::from_str::<CheckEmailRequest>(r#"{"to_email":"cache@reacher.email"}"#)
			.unwrap();
		...
		assert_eq!(first_body, second_body, "Cached response should match original response");
...
mod v0_bulk_coverage {
	...
	async fn test_v0_bulk_results_query() {
		...
		sqlx::query("INSERT INTO email_results (job_id, result) VALUES ($1, $2)")
			...
			.execute(db.pool()).await.unwrap();
		sqlx::query("INSERT INTO email_results (job_id, result) VALUES ($1, $2)")
			.bind(job_id).bind(&serde_json::json!({"is_reachable": "invalid", "input": "c@d.com"}))
			.execute(db.pool()).await.unwrap();

		// Query with limit/offset (mirrors v0/bulk/results/mod.rs logic)
		let rows = sqlx::query("SELECT result FROM email_results WHERE job_id = $1 ORDER BY id LIMIT $2 OFFSET $3")
			.bind(job_id).bind(1i64).bind(0i64)
			.fetch_all(db.pool()).await.unwrap();
		assert_eq!(rows.len(), 1);

		let rows_all = sqlx::query("SELECT result FROM email_results WHERE job_id = $1 ORDER BY id")
	...
	async fn test_v0_bulk_job_creation() {
		...
		let row = sqlx::query("INSERT INTO bulk_jobs (total_records) VALUES ($1) RETURNING id")
			.bind(5i32).fetch_one(db.pool()).await.unwrap();
...
mod v1_bulk_coverage {
	use serial_test::serial;
	use crate::test_helpers::{TestDb, insert_job, insert_task, safe_result, invalid_result};
	use sqlx::Row;
	...
	async fn test_v1_bulk_pre_create_tasks() {
		...
		let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM v1_task_result WHERE job_id = $1")
			.bind(job_id).fetch_one(db.pool()).await.unwrap();
		assert_eq!(count, 3);
	...
	async fn test_v1_bulk_progress_aggregation() {
		...
		insert_task(db.pool(), job_id, "completed", None, Some(safe_result()), None).await;
		insert_task(db.pool(), job_id, "completed", None, Some(invalid_result()), None).await;
		insert_task(db.pool(), job_id, "queued", None, None, None).await;
	...
	async fn test_v1_bulk_results_pagination() {
		...
		// First page (limit 2)
		let rows = sqlx::query("SELECT id, result FROM v1_task_result WHERE job_id = $1 ORDER BY id LIMIT $2")
			.bind(job_id).bind(2i64)
			.fetch_all(db.pool()).await.unwrap();
		assert_eq!(rows.len(), 2);

		// Offset page
		let last_id: i32 = rows.last().unwrap().get("id");
		let next = sqlx::query("SELECT id FROM v1_task_result WHERE job_id = $1 AND id > $2 ORDER BY id LIMIT $3")
			.bind(job_id).bind(last_id).bind(2i64)
			.fetch_all(db.pool()).await.unwrap();
...
mod v1_jobs_coverage {
	use serial_test::serial;
	use crate::test_helpers::{TestDb, insert_job, insert_task, insert_event, safe_result};
	use sqlx::Row;
	...
	async fn test_events_recording_and_query() {
		...
		let task_id = insert_task(db.pool(), job_id, "completed", None, Some(safe_result()), None).await;
		...
		let events = sqlx::query("SELECT event_type, task_id FROM job_events WHERE job_id = $1 ORDER BY created_at")
			.bind(job_id).fetch_all(db.pool()).await.unwrap();
	...
	async fn test_cursor_based_results() {
		...
		let mut ids = vec![];
		for _ in 0..5 {
			let id = insert_task(db.pool(), job_id, "completed", None, Some(safe_result()), None).await;
			ids.push(id);
		}
...
mod do_check_email_work_coverage {
	...
	use reacher_backend::worker::consume::{setup_rabbit_mq, CHECK_EMAIL_QUEUE};
	use reacher_backend::worker::do_work::{do_check_email_work, CheckEmailJobId, CheckEmailTask, TaskMetadata, RetryPolicy};
	use futures::StreamExt;
	use lapin::options::*;
	use lapin::types::FieldTable;
	use lapin::BasicProperties;
	...
	async fn test_do_check_email_work_success_path() {
		...
		let rmq_config = RabbitMQConfig { url: rmq_url(), concurrency: 4 };
		let channel = Arc::new(setup_rabbit_mq("dowork-success", &rmq_config).await.unwrap());
		...
		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "invalid-syntax".into(), ..Default::default()
			},
			job_id: CheckEmailJobId::Bulk(job_id),
			webhook: None,
			metadata: Some(TaskMetadata {
				tenant_id: None, request_id: None, correlation_id: None,
				created_by: None, retry_policy: Some(RetryPolicy { max_retries: 0, backoff_seconds: 1, backoff_multiplier: 1.0 }),
				dedupe_key: None, task_db_id: Some(task_db_id),
			}),
		};
		let json = serde_json::to_vec(&task).unwrap();
		channel.basic_publish("", CHECK_EMAIL_QUEUE, BasicPublishOptions::default(), &json,
		...
		let mut consumer = channel.basic_consume(CHECK_EMAIL_QUEUE, "dowork-test",
			BasicConsumeOptions::default(), FieldTable::default()).await.unwrap();
		...
		assert!(result.is_ok(), "do_check_email_work failed: {:?}", result.err());

		// Verify task state was updated to completed
		let state: String = sqlx::query("SELECT task_state::TEXT FROM v1_task_result WHERE id = $1")
			.bind(task_db_id).fetch_one(db.pool()).await.unwrap().get(0);
	...
	async fn test_do_check_email_work_dead_letter_path() {
		...
		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "test@nonexistent-domain-xyz123.tld".into(), ..Default::default()
			},
			job_id: CheckEmailJobId::Bulk(job_id),
			webhook: None,
			metadata: Some(TaskMetadata {
		...
		let mut consumer = channel.basic_consume(CHECK_EMAIL_QUEUE, "dowork-dl-test",
			BasicConsumeOptions::default(), FieldTable::default()).await.unwrap();
		...
		let state: String = sqlx::query("SELECT task_state::TEXT FROM v1_task_result WHERE id = $1")
			.bind(task_db_id).fetch_one(db.pool()).await.unwrap().get(0);
		assert!(state == "completed" || state == "dead_lettered",
			"Expected completed or dead_lettered, got: {}", state);
	}
}
...
mod do_check_email_work_retry_coverage {
	...
	fn rmq_url() -> String { std::env::var("TEST_AMQP_URL").unwrap_or_else(|_| "amqp://guest:guest@127.0.0.1:35672".into()) }
	fn db_url() -> String { std::env::var("TEST_DATABASE_URL").unwrap_or_else(|_| "postgres://postgres:postgres@127.0.0.1:25432/reacher_test".into()) }

	async fn make_config() -> Arc<BackendConfig> {
		let mut config = BackendConfig::empty();
		config.storage = Some(StorageConfig::Postgres(PostgresConfig { db_url: db_url(), extra: None }));
		config.worker = WorkerConfig { enable: true, rabbitmq: Some(RabbitMQConfig { url: rmq_url(), concurrency: 4 }), webhook: None };
		config.connect().await.unwrap();
		Arc::new(config)
	}
	...
	async fn test_do_check_email_work_single_shot() {
		...
		let reply_queue = channel.queue_declare("", QueueDeclareOptions { auto_delete: true, durable: false, exclusive: true, ..Default::default() }, FieldTable::default()).await.unwrap();
		let correlation_id = uuid::Uuid::new_v4().to_string();

		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput { to_email: "bad".into(), ..Default::default() },
			job_id: CheckEmailJobId::SingleShot,
			webhook: None,
			metadata: None,
		};
		...
		let mut consumer = channel.basic_consume(CHECK_EMAIL_QUEUE, "dowork-ss-con",
			BasicConsumeOptions::default(), FieldTable::default()).await.unwrap();
		let delivery = tokio::time::timeout(std::time::Duration::from_secs(5), consumer.next())
			.await.unwrap().unwrap().unwrap();
		let received: CheckEmailTask = serde_json::from_slice(&delivery.data).unwrap();
	...
	async fn test_do_check_email_work_with_retry_policy() {
		...
		let task = CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput { to_email: "user@nonexistent-domain-abc999.tld".into(), ..Default::default() },
			job_id: CheckEmailJobId::Bulk(job_id),
			webhook: None,
			metadata: Some(TaskMetadata {
				tenant_id: None, request_id: None, correlation_id: None, created_by: None,
				retry_policy: Some(RetryPolicy { max_retries: 5, backoff_seconds: 1, backoff_multiplier: 1.0 }),
				dedupe_key: None, task_db_id: Some(task_db_id),
			}),
		};
		let json = serde_json::to_vec(&task).unwrap();
		...
		let mut consumer = channel.basic_consume(CHECK_EMAIL_QUEUE, "dowork-retry-con",
			BasicConsumeOptions::default(), FieldTable::default()).await.unwrap();
		let delivery = tokio::time::timeout(std::time::Duration::from_secs(5), consumer.next())
			.await.unwrap().unwrap().unwrap();
		let received: CheckEmailTask = serde_json::from_slice(&delivery.data).unwrap();
		...
		assert!(
			...
			"Expected completed or retrying, got: {}", state
		);
	}
}
...
mod postgres_storage_coverage {
	...
	use reacher_backend::worker::do_work::{CheckEmailJobId, CheckEmailTask, TaskMetadata, TaskError};
	...
	fn make_task(metadata: Option<TaskMetadata>) -> CheckEmailTask {
		CheckEmailTask {
			input: check_if_email_exists::CheckEmailInput {
				to_email: "store@test.com".into(), ..Default::default()
			},
			job_id: CheckEmailJobId::SingleShot,
			webhook: None,
			metadata,
	...
	async fn test_new_connects_and_migrates() {
		...
		// Verify we can query
		let row = sqlx::query("SELECT 1 as v").fetch_one(&storage.pg_pool).await.unwrap();
	...
	async fn test_store_err_result() {
		let storage = PostgresStorage::new(&db_url(), None).await.unwrap();
		let task = make_task(None);
		let err: Result<CheckEmailOutput, TaskError> = Err(TaskError::Lapin(lapin::Error::InvalidChannel(0)));
	...
	async fn test_store_with_extra() {
		...
		let storage = PostgresStorage::new(&db_url(), extra.clone()).await.unwrap();
		assert_eq!(storage.get_extra(), extra);

		let task = make_task(None);
		let output = CheckEmailOutput {
			input: "store@test.com".into(), is_reachable: Reachable::Invalid, ..Default::default()
		};
	...
	async fn test_store_with_tenant_id() {
		...
		let task = make_task(Some(TaskMetadata {
			...
			request_id: None, correlation_id: None, created_by: None,
