...
async fn seed_fixtures(pool: &PgPool) -> HarnessFixtures {
...
async fn build_runtime(db_url: &str, amqp_url: &str) -> HarnessRuntime {
	HarnessRuntime {
...
fn harness_keys(cases: &[HarnessCase]) -> BTreeSet<(String, String)> {
	cases
		.iter()
...
fn auth_failure_cases() -> Vec<HarnessCase> {
	vec![
		case!(
			"GET",
			"/v1/me",
...
fn validation_failure_cases() -> Vec<HarnessCase> {
	vec![
		case!(
			"GET",
...
fn missing_resource_cases() -> Vec<HarnessCase> {
	vec![
		case!(
			"GET",
			"/v1/jobs/999999",
...
fn conflict_cases() -> Vec<HarnessCase> {
	vec![
		case!(
			"POST",
			"/v1/jobs/{job_id}/cancel",
