 ...
 * the 30px chip pill (`rounded-none`, chip surface tokens) shared by
 ...
 * `Combobox`, so `rounded-none` / height / dark surface and the chip `--text-body`
...
export function ChipCombobox({ className, ...props }: ComboboxProps) {
  return (
    <Combobox
      {...props}
      className={cn(
        'h-[30px] rounded-none text-[var(--text-body)] dark:bg-[var(--surface-4)]',
        className
      )}
    />
  )
