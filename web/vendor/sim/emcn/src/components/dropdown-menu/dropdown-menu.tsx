...
const MENU_ROW_RADIUS_CLASS = 'rounded-none'
 ...
 *
 * `rounded-none`/`rounded-none` here are the platform's two-tier radius convention,
 ...
 */
const CONTENT_BASE_CLASSES = `z-[var(--z-popover)] ${MENU_MAX_HEIGHT_CLASS} min-w-[8rem] origin-[--radix-dropdown-menu-content-transform-origin] overflow-y-auto overflow-x-hidden overscroll-none rounded-none border border-[var(--border)] bg-[var(--bg)] p-1 text-[var(--text-body)] shadow-xs`
...
>(({ className, onClick, onPointerDown, ...props }, ref) => (
  <button
    ...
    className={cn(
      'flex size-[18px] shrink-0 items-center justify-center rounded-none outline-hidden [&_svg]:pointer-events-none [&_svg]:size-[16px] [&_svg]:text-[var(--text-icon)]',
      className
    )}
    {...props}
  />
