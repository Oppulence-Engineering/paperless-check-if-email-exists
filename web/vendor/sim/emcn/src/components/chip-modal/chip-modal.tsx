...
>(({ className, children, onKeyDown, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      'flex min-h-0 w-full flex-col rounded-none border border-[var(--border-muted)] bg-[var(--surface-4)] p-[3px] text-small dark:bg-[var(--surface-5)]',
      className
    )}
    onKeyDown={(event) => {
      onKeyDown?.(event)
    ...
    {...props}
  >
    <div className='flex min-h-0 flex-1 flex-col overflow-hidden rounded-none border border-[var(--border-1)] bg-[var(--bg)]'>
      {children}
    </div>
  </div>
))
 ...
 * fill, `--border-1` border, `rounded-none`) so it stacks as a visual peer with
...
function ChipModalFileControl({
  ...
  return (
    <button
      ...
      className={cn(
        'flex w-full min-w-0 flex-col items-center justify-center gap-0.5 rounded-none border border-[var(--border-1)] border-dashed bg-[var(--surface-5)] px-2 py-2.5 text-center outline-hidden transition-colors hover-hover:border-[var(--surface-7)] disabled:cursor-not-allowed disabled:opacity-50 dark:bg-[var(--surface-4)]',
