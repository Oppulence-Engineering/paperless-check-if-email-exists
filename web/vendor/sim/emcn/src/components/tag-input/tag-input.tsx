...
const tagInputVariants = cva(
  'flex w-full cursor-text flex-wrap gap-2 overflow-y-auto border border-[var(--border-1)] bg-[var(--surface-5)] px-2 transition-colors',
  {
    variants: {
      variant: {
        default:
          'items-center rounded-none py-1.5 focus-within:outline-hidden dark:bg-[var(--surface-5)]',
        block:
          'min-h-[112px] content-start items-start rounded-none py-2 focus-within:outline-hidden dark:bg-[var(--surface-4)]',
      },
    },
    defaultVariants: {
      variant: 'default',
  ...
  (
    ...
    return (
      ...
      >
        ...
        {isDragging && (
          <div className='absolute inset-0 flex items-center justify-center rounded-none bg-[color-mix(in_srgb,var(--surface-5)_90%,transparent)]'>
