...
These features depend on external OAuth flows, third-party APIs, or partner infrastructure. They cannot be shipped without establishing integration partnerships first.

| # | Area | Feature | External Dependency | Priority |
|---|---|---|---|---|
| 7 | List Cleaning | One-click suppression sync back to ESPs and CRMs | OAuth to each ESP/CRM | Critical |
| 10 | Integrations | Native HubSpot sync | HubSpot API + OAuth | Critical |
| 11 | Integrations | Native Salesforce sync | Salesforce API + OAuth | Critical |
| 12 | Integrations | Zapier connector | Zapier Developer Platform | Critical |
| 13 | Integrations | Google Sheets sync | Google Sheets API + OAuth | Critical |
| 37 | List Cleaning | CSV and CRM column mapping wizard | CRM schema discovery APIs | High |
| 45 | Finder / Enrichment | Company identification from domain | External company database | High |
| 46 | Finder / Enrichment | Basic firmographic enrichment | External enrichment API (Clearbit, Apollo) | High |
| 47 | Finder / Enrichment | Job title enrichment | External enrichment API | High |
| 49 | Finder / Enrichment | People search by company | External people database | High |
| 52 | Finder / Enrichment | Seniority and department enrichment | External enrichment API | High |
| 53 | Finder / Enrichment | LinkedIn and company-page matching | LinkedIn API or scraping | High |
| 54 | Integrations | Native Outreach sync | Outreach API + OAuth | High |
| 55 | Integrations | Native Salesloft sync | Salesloft API + OAuth | High |
| 56 | Integrations | Native Marketo sync | Marketo API + OAuth | High |
| 57 | Integrations | Native Mailchimp sync | Mailchimp API + OAuth | High |
| 58 | Integrations | Native Klaviyo sync | Klaviyo API + OAuth | High |
| 59 | Integrations | Native Pipedrive sync | Pipedrive API + OAuth | High |
| 60 | Integrations | Native Close sync | Close API + OAuth | High |
| 61 | Integrations | Make connector | Make Developer Platform | High |
| 62 | Integrations | n8n templates / connector | n8n node ecosystem | High |
| 63 | Integrations | Reverse ETL and warehouse destinations | Warehouse APIs (Fivetran, Census) | High |
| 64 | Integrations | Snowflake and BigQuery export targets | Snowflake/BigQuery APIs | High |
| 93 | Automation | Slack and Microsoft Teams alerts | Slack/Teams webhook APIs | Medium |
| 97 | Finder / Enrichment | Social profile enrichment | Social platform APIs | Medium |
| 98 | Finder / Enrichment | Technographic enrichment | External technographic API (BuiltWith, Wappalyzer) | Medium |
| 99 | Finder / Enrichment | Enrichment source attribution | Depends on enrichment integrations above | Medium |
