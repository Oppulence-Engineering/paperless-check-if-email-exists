/*
Reacher

### What is Reacher?  Reacher is a robust, open-source email verification API service available as both a SaaS and self-hosted solution.

API version: 0.11.0
Contact: amaury@reacher.email
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package reacher

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ConfidenceExplanation type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConfidenceExplanation{}

// ConfidenceExplanation struct for ConfidenceExplanation
type ConfidenceExplanation struct {
	Factors []string `json:"factors"`
	Level string `json:"level"`
	Score int32 `json:"score"`
}

type _ConfidenceExplanation ConfidenceExplanation

// NewConfidenceExplanation instantiates a new ConfidenceExplanation object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConfidenceExplanation(factors []string, level string, score int32) *ConfidenceExplanation {
	this := ConfidenceExplanation{}
	this.Factors = factors
	this.Level = level
	this.Score = score
	return &this
}

// NewConfidenceExplanationWithDefaults instantiates a new ConfidenceExplanation object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConfidenceExplanationWithDefaults() *ConfidenceExplanation {
	this := ConfidenceExplanation{}
	return &this
}

// GetFactors returns the Factors field value
func (o *ConfidenceExplanation) GetFactors() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.Factors
}

// GetFactorsOk returns a tuple with the Factors field value
// and a boolean to check if the value has been set.
func (o *ConfidenceExplanation) GetFactorsOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Factors, true
}

// SetFactors sets field value
func (o *ConfidenceExplanation) SetFactors(v []string) {
	o.Factors = v
}

// GetLevel returns the Level field value
func (o *ConfidenceExplanation) GetLevel() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Level
}

// GetLevelOk returns a tuple with the Level field value
// and a boolean to check if the value has been set.
func (o *ConfidenceExplanation) GetLevelOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Level, true
}

// SetLevel sets field value
func (o *ConfidenceExplanation) SetLevel(v string) {
	o.Level = v
}

// GetScore returns the Score field value
func (o *ConfidenceExplanation) GetScore() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Score
}

// GetScoreOk returns a tuple with the Score field value
// and a boolean to check if the value has been set.
func (o *ConfidenceExplanation) GetScoreOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Score, true
}

// SetScore sets field value
func (o *ConfidenceExplanation) SetScore(v int32) {
	o.Score = v
}

func (o ConfidenceExplanation) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConfidenceExplanation) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["factors"] = o.Factors
	toSerialize["level"] = o.Level
	toSerialize["score"] = o.Score
	return toSerialize, nil
}

func (o *ConfidenceExplanation) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"factors",
		"level",
		"score",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varConfidenceExplanation := _ConfidenceExplanation{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	err = decoder.Decode(&varConfidenceExplanation)

	if err != nil {
		return err
	}

	*o = ConfidenceExplanation(varConfidenceExplanation)

	return err
}

type NullableConfidenceExplanation struct {
	value *ConfidenceExplanation
	isSet bool
}

func (v NullableConfidenceExplanation) Get() *ConfidenceExplanation {
	return v.value
}

func (v *NullableConfidenceExplanation) Set(val *ConfidenceExplanation) {
	v.value = val
	v.isSet = true
}

func (v NullableConfidenceExplanation) IsSet() bool {
	return v.isSet
}

func (v *NullableConfidenceExplanation) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConfidenceExplanation(val *ConfidenceExplanation) *NullableConfidenceExplanation {
	return &NullableConfidenceExplanation{value: val, isSet: true}
}

func (v NullableConfidenceExplanation) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConfidenceExplanation) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


